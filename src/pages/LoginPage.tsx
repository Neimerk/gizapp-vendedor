...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (checkBlock()) return; // bloqueado por excesso de tentativas
        ...
        <div className="rounded-2xl border border-[#e2e8f0] bg-white p-7" style={{ boxShadow: "0 4px 24px rgba(0,0,0,0.07)" }}>
          {blocked ? (
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <div>
                <p className="font-black text-[#0f172a]">Conta temporariamente bloqueada</p>
                <p className="mt-2 text-sm text-[#64748b]">
                  Muitas tentativas de login falhadas. Tente novamente em{" "}
                  <span className="font-black text-red-600">{remainingMin} min</span>.
                </p>
              </div>
              <p className="text-xs text-[#94a3b8]">Se esqueceu sua senha, entre em contato com o suporte.</p>
            </div>
          ) : (
